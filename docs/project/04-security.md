...
- The staging site (`badminton.polardev.org`) is refreshed from production every night at 04:00. The copy itself is faithful, so what arrives is real names, emails, phone numbers, officer notes and fee records.
  ...
  - *Still open:* the snapshot script runs from a plain checkout on the Pi (`~/ssd/Deploy/badminton-staging`, tracking `deploy/docker-staging`) that nothing auto-updates, so **the scrub is not protecting anything until that checkout is pulled**. Until then there are two full copies of the membership on the public internet, not one, behind the same login and nothing more, and any breach is scoped to both.
