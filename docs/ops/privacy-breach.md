...
| **Staging Postgres** (`supabase-staging-db`, same Pi) | A full unscrubbed copy of prod, refreshed nightly at 04:00 | Real names, emails and phones, reachable on the public internet at `badminton.polardev.org`. **Scope every incident to staging as well as prod.** The refresh drops and reloads `public`, so an anonymisation on prod does reach staging, but only at the next 04:00 — a deleted member is still readable here for up to 24 hours afterwards |
