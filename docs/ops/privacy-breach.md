...
| **The off-site copy on the Mac** | Same | **Encrypted at rest since 2026-09-22** (`age`). If the private key is off that machine, a stolen or seized Mac is ciphertext and not a disclosure. Two caveats, and neither is optional: anything dated **before 2026-09-22** was plaintext, and the protection is only real while the key stays off the disk. **Check the last run's output before scoping this out** (`launchctl list \| grep badminton`, and the log): the job warns by name when it finds a private key on the Mac, and while that warning is firing this row is a plaintext member database. Bounded since 2026-09-21 to a 14-day window |
