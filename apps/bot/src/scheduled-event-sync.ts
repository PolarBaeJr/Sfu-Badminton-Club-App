...
// Keeps the server's Events tab in step with something the club schedules:
// tournaments (tournament-events.ts) and club events (club-events.ts). Each is
// a thin source over this one loop.
//
...
// next tick, which somebody sees and mentions. Recording first would mean an
// event that says "announced" and never was, with nothing to find it by.
//
...
export interface ScheduledEventRunResult {
  created: number;
  updated: number;
  cancelled: number;
...
export interface ScheduledEventAction {
  ...
  /** The tournament or club event this action is about. */
  sourceId: string;
  ...
  startsAt: string;
  endsAt: string;
  /** What the source ITSELF says. Recorded, so the change detector stays stable. */
...
export interface ScheduledEventSource {
  /** Used in log lines, e.g. 'tournament events'. */
  label: string;
  fetchActions(guildId: string): Promise<{
    actions: ScheduledEventAction[];
    skipped: { sourceId: string; reason: string }[];
  }>;
  /** Record an event Discord has ALREADY accepted. */
  record(guildId: string, discordEventId: string, action: ScheduledEventAction): Promise<unknown>;
  /** Forget a mapping, after the Discord event is gone. */
  clear(guildId: string, sourceId: string): Promise<unknown>;
}

// What Discord shows when there is no location. Not an empty string:
...
export const FALLBACK_LOCATION = 'SFU Badminton Club';

export async function runScheduledEventSync(
  source: ScheduledEventSource
): Promise<ScheduledEventRunResult> {
  const result: ScheduledEventRunResult = {
    created: 0,
    updated: 0,
    cancelled: 0,
    stale: 0,
  ...
  if (!token) {
    console.error(`[bot] ${label}: DISCORD_BOT_TOKEN is not set`);
    return result;
  }
  ...
  for (const guildId of registry.keys()) {
    let actions: ScheduledEventAction[];
    let skipped: { sourceId: string; reason: string }[];
    try {
      ({ actions, skipped } = await source.fetchActions(guildId));
    } catch (error) {
      // One guild's failure must not abort the others.
      console.error(`[bot] ${label}: could not read actions for ${guildId}:`, error);
      result.failed += 1;
      continue;
    }
    ...
    for (const s of skipped) {
      ...
      console.log(`[bot] ${label}: skipping ${s.sourceId} (${s.reason})`);
    }
    ...
    try {
      if (!(await api.hasManageEvents(guildId))) {
        console.error(
          `[bot] ${label}: missing MANAGE_EVENTS in guild ${guildId}. ` +
      ...
      console.error(
        `[bot] ${label}: could not check permissions in ${guildId}, trying anyway:`,
        error
      );
    }
    ...
    for (const action of actions) {
      ...
      if (action.kind === 'cancel') {
        ...
        if (!gone) {
          console.error(
            `[bot] ${label}: could not cancel ${action.discordEventId} ` +
              `for ${action.sourceId}`
          );
          result.failed += 1;
          continue;
        ...
        try {
          await source.clear(guildId, action.sourceId);
          result.cancelled += 1;
        } catch (error) {
          ...
          console.error(
            `[bot] ${label}: CANCELLED but not cleared (${action.sourceId}):`,
            error
          );
          result.cancelled += 1;
        }
      ...
      if (action.kind === 'create') {
        ...
        // every fifteen minutes until the source ends. The mapping keeps its
        // dead id, and the cancel path clears it later, where a 404 delete
        // already counts as done.
        if (outcome === 'gone') {
          stale = true;
          console.warn(
            `[bot] ${label}: event ${action.discordEventId} for ` +
              `${action.sourceId} is gone (deleted by hand?). Recording the update ` +
      ...
      try {
        // RECORDS THE SOURCE'S OWN TIMES, not the ones just sent. The two
        ...
        await source.record(guildId, discordEventId, action);
      } catch (error) {
        ...
        console.error(
          `[bot] ${label}: ${action.kind}d but NOT recorded ` +
            `(${action.sourceId}/${discordEventId}). A duplicate may appear ` +
            'on the next run:',
          error
        );
      }
