...
describe('the player top bar', () => {
  it('shows an approved member every entry', () => {
    ...
    expect(labels(desktopEntries(true))).toEqual(['Feed', 'Challenges', 'Events', 'Stats']);
  });
  ...
  it('shows a pending member no Challenges and no Events', () => {
    ...
    expect(labels(entries)).toEqual(['Feed', 'Stats']);
...
describe('the nav with a feature switched off', () => {
  ...
  it('drops the Events menu on both bars when tournaments and club events are off', () => {
    expect(labels(desktopEntries(true, off('tournaments', 'events')))).toEqual(['Feed', 'Challenges', 'Stats']);
  ...
  it('keeps Events with only club events in it when tournaments alone are off', () => {
    expect(labels(desktopEntries(true, off('tournaments')))).toEqual(['Feed', 'Challenges', 'Events', 'Stats']);
  ...
  it('keeps Events for a holder of page.access.tournaments, who can still open the pages', () => {
    expect(labels(desktopEntries(true, off('tournaments', 'events'), ['tournaments']))).toEqual(['Feed', 'Challenges', 'Events', 'Stats']);
  ...
  it('drops Events for a holder of a different feature key', () => {
    expect(labels(desktopEntries(true, off('tournaments', 'events'), ['challenges']))).toEqual(['Feed', 'Challenges', 'Stats']);
  });
  ...
  it('drops the Challenges link when challenges are off', () => {
    expect(labels(desktopEntries(true, off('challenges')))).toEqual(['Feed', 'Events', 'Stats']);
  ...
  it('shrinks a menu rather than dropping it while something in it is still on', () => {
    ...
    expect(labels(desktopEntries(true, off('leaderboard')))).toEqual(['Feed', 'Challenges', 'Events', 'Stats']);
  });
