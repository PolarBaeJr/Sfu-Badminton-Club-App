    ...
    <div className="space-y-6 animate-pulse">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="h-4 w-40 bg-[var(--bg-secondary)] rounded mt-2" />
        </div>
        <div className="flex gap-2">
          <div className="h-10 w-28 bg-[var(--bg-secondary)] rounded" />
          <div className="h-10 w-28 bg-[var(--bg-secondary)] rounded" />
        </div>
      </div>
      ...
      <div className="flex gap-2 border-b border-[var(--border-primary)] pb-2">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="h-9 w-24 bg-[var(--bg-secondary)] rounded" />
        ))}
      </div>
