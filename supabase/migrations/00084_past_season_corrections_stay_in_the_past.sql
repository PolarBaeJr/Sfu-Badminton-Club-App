-- Corrections to a FINISHED season stop moving today's ladder.
--
-- Per the owner: allow them, but recalculate them where they belong.
--
-- The live `ratings` table is not a running total an old match can be added
-- back into. At a season rollover it is archived to season_final_ratings and
-- then REBASED — compressed toward the mean, or under the 'full' policy reset
-- to the baseline with every counter zeroed. Subtracting a two-year-old +20
-- from a rating that has since been reset to 400 does not correct the old
-- season; it just makes the current one wrong as well.
--
-- So a correction to a match whose season is closed goes where that match
-- actually counted: that season's archived final ladder. Elo only — the archive
-- holds nothing else, and the statistics counters in `ratings` may already have
-- been zeroed, so there is no coherent adjustment to make to them.
--
-- The snapshot records `archived_season_id`, and carries no `won`. That is the
-- pre-00070 shape, which the reversal already reads as "this one moved Elo and
-- nothing else" — exactly true here. It is recorded deliberately, not by
-- omission, and the reversal keys off the season id to put the delta back in
-- the same place it came from.
--
-- A match in the ACTIVE season, or one whose season was never archived, behaves
-- exactly as before.
--
-- Depends on 00082 and 00083.
...
BEGIN
  ...
  -- Which season's ladder does this match belong to, and has that ladder
  -- already been closed off?
  --
  -- A correction to a FINISHED season must not move today's numbers. The live
  -- `ratings` table is not a running total an old match can be added back into:
  -- at rollover it is archived to season_final_ratings and then rebased —
  -- compressed, or reset to the baseline outright with every counter zeroed.
  -- Subtracting a two-year-old +20 from a rating that was reset to 400 does not
  -- correct anything, it makes today's ladder wrong too.
  --
  -- So the delta goes where the match actually counted: that season's archived
  -- final ladder. Elo only — the archive holds nothing else, and the statistics
  -- counters may already have been zeroed.
  --
  -- Placed here, immediately before the loop, and NOT up beside the match read:
  -- everything above this point can still return early, and a detection that
  -- lives inside one of those branches silently never runs.
  ...
  v_archived := v_season_id IS NOT NULL
    AND NOT EXISTS (SELECT 1 FROM seasons WHERE id = v_season_id AND active_flag)
    AND EXISTS (SELECT 1 FROM season_final_ratings WHERE season_id = v_season_id);
  ...
  FOR v_entry IN SELECT * FROM jsonb_array_elements(p_entries) LOOP
    ...
    IF v_archived THEN
      -- Closed season: adjust that season's archived ladder and nothing else.
      UPDATE season_final_ratings sfr
         SET singles_elo = CASE WHEN p_discipline = 'singles'
               THEN LEAST(GREATEST(sfr.singles_elo + (v_entry->>'delta')::integer, v_lo), v_hi)
               ELSE sfr.singles_elo END,
             doubles_elo = CASE WHEN p_discipline = 'doubles'
               THEN LEAST(GREATEST(sfr.doubles_elo + (v_entry->>'delta')::integer, v_lo), v_hi)
               ELSE sfr.doubles_elo END,
             archived_at = NOW()
       WHERE sfr.season_id = v_season_id AND sfr.player_id = v_player_id;
      IF NOT FOUND THEN
        RAISE EXCEPTION 'Player % has no archived rating for season % — cannot correct that season''s match %',
          v_player_id, v_season_id, p_match_id;
      END IF;

      SELECT CASE WHEN p_discipline = 'singles' THEN singles_elo ELSE doubles_elo END
        INTO v_new_elo FROM season_final_ratings
       WHERE season_id = v_season_id AND player_id = v_player_id;
      v_applied_delta := (v_entry->>'delta')::integer;

      -- No `won` on purpose: that is the pre-00070 snapshot shape, which the
      -- reversal reads as "this one moved Elo and nothing else" — exactly true
      -- here. archived_season_id tells the reversal where to put it back.
      v_snapshot_entries := v_snapshot_entries || jsonb_build_object(
        'player_id', v_entry->>'player_id',
        'before',    v_new_elo - v_applied_delta,
        'after',     v_new_elo,
        'delta',     v_applied_delta,
        'archived_season_id', v_season_id
      );
      CONTINUE;
    END IF;
...
BEGIN
  ...
  FOR v_entry IN SELECT * FROM jsonb_array_elements(v_snapshot->'entries') LOOP
    ...
    -- A closed season's correction went into that season's ARCHIVED ladder, and
    -- the entry says so. Take it back out of the same place. Reversing it out of
    -- the live table instead would move today's numbers by a delta that was
    -- never added to them.
    IF (v_entry ? 'archived_season_id') THEN
      UPDATE season_final_ratings sfr
         SET singles_elo = CASE WHEN v_discipline = 'singles'
               THEN sfr.singles_elo - (v_entry->>'delta')::integer ELSE sfr.singles_elo END,
             doubles_elo = CASE WHEN v_discipline = 'doubles'
               THEN sfr.doubles_elo - (v_entry->>'delta')::integer ELSE sfr.doubles_elo END,
             archived_at = NOW()
       WHERE sfr.season_id = (v_entry->>'archived_season_id')::uuid
         AND sfr.player_id = v_player_id;
      IF NOT FOUND THEN
        RAISE EXCEPTION 'Player % has no archived rating for season % — cannot reverse match %',
          v_player_id, v_entry->>'archived_season_id', p_match_id;
      END IF;
      CONTINUE;
    END IF;
