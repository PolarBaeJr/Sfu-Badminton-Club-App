-- 00216 — two entrants in one draw are not one entrant
--
-- merge_players refuses a same-event collision only when the removed account is
-- in a PAIR (00207). Two PARTICIPANT rows in the same event were never refused.
-- What happens instead, today:
--
--   1. The participant remap excludes the removed account's row with a
--      NOT EXISTS on the survivor's row in that event, so the row is not
--      repointed. It is counted into v_dropped and nothing else.
--   2. The tournament_matches remap then sends every match reference held by
--      the dropped row onto the SURVIVOR's row in that same event.
--   3. DELETE FROM players cascades the dropped row away.
--
-- Step 2 is the right answer when only one of the two rows ever played: a
-- person registered twice, played under one entry, and the duplicate is the
-- thing to discard. It is the wrong answer when BOTH rows played. The draw
-- seeded them as two distinct entrants; after the remap one participant row
-- holds both entrants' matches and therefore sits in two bracket slots.
--
-- Nothing downstream notices:
--
--   * The self-play sweep further down looks for a match whose two sides are
--     the same participant. Two entrants on opposite halves of a draw never
--     played each other, so it finds nothing and no elo_review flag is raised.
--   * Finalisation's slot check passes -- the survivor's row is a legitimate
--     entrant of the event -- and its single positionMap entry is overwritten
--     as the reader walks the collapsed matches, so the published placing is
--     whichever the last write happened to be.
--   * The placings are written under the event field lock (00212/00215), so no
--     later recompute repairs it. The corruption is durable.
--
-- A standings recompute cannot resolve this, because the information that is
-- lost -- which of the two entrants each result belonged to -- is destroyed by
-- the remap itself. Which entry keeps the draw is an operator's decision, the
-- same reasoning 00207 gives for refusing the pair collision. So this refuses,
-- and names the event so the refusal is actionable.
--
-- Deliberately scoped to rows that PLAYED. A duplicate registration in a draft
-- or ungenerated event holds no match references, keeps dropping exactly as it
-- does today, and stays a one-click merge. That is the common duplicate-account
-- case and it must not become a two-step chore to close a corruption that only
-- exists once a draw has been generated and results recorded.
--
-- The body below is 00207's verbatim, with one block added after the pair
-- refusal. It was spliced programmatically and diffed against the extract to
-- prove no other line moved.
...
BEGIN
  ...
  -- club_ledger's three: NEW in 00207. 00159 folded the old fee tables into this
  -- one and never classified its columns, so a merge whose removed account had
  -- ever marked, paid, or reimbursed a ledger line died on a raw 23503.
  UPDATE club_ledger            SET marked_by         = p_keep WHERE marked_by         = p_remove;
  UPDATE club_ledger            SET paid_by           = p_keep WHERE paid_by           = p_remove;
  UPDATE club_ledger            SET reimbursed_by     = p_keep WHERE reimbursed_by     = p_remove;
  ...
  -- ELEVEN MORE, NEW in 00207, and this is the silent half of the finding. Every
  -- one of these is ON DELETE SET NULL, so before this migration the merge did
  -- not fail on them -- it succeeded and quietly blanked them. An admin note
  -- lost its author, a dispute lost who claimed it, a permission baseline lost
  -- who created it. Nothing raised, nothing was counted in rows_discarded, and
  -- the loss is unrecoverable once the loser's id is gone.
  UPDATE discord_link_tokens        SET consumed_by = p_keep WHERE consumed_by = p_remove;
  UPDATE disputes                   SET claimed_by  = p_keep WHERE claimed_by  = p_remove;
  UPDATE event_waiver_templates     SET updated_by  = p_keep WHERE updated_by  = p_remove;
  UPDATE match_admin_notes          SET author_id   = p_keep WHERE author_id   = p_remove;
  UPDATE permission_baselines       SET created_by  = p_keep WHERE created_by  = p_remove;
  UPDATE permission_baselines       SET updated_by  = p_keep WHERE updated_by  = p_remove;
  UPDATE tournament_match_notes     SET author_id   = p_keep WHERE author_id   = p_remove;
  UPDATE tournament_pair_notes      SET author_id   = p_keep WHERE author_id   = p_remove;
  UPDATE tournament_participant_notes SET author_id = p_keep WHERE author_id   = p_remove;
  UPDATE walkover_admin_notes       SET author_id   = p_keep WHERE author_id   = p_remove;
  ...
  -- ---- tournament_pairs identity columns (NEW in 00207) ----
  --
  -- player1_id and player2_id are ON DELETE NO ACTION, and nothing above moves
  -- them, so before this migration a merge whose removed account had EVER been
  -- half of a doubles team died at the final DELETE with a bare
  --   23503 ... violates foreign key constraint "tournament_pairs_player1_id_fkey"
  -- and no explanation of what to do about it. Reproduced on staging.
  --
  -- These are not attribution columns like added_by. They say who is ON the
  -- team, so they are repointed only when the survivor is not already present
  -- in that event -- otherwise the repoint would either fold both halves of a
  -- team into one person or put one human in the same event twice, which is the
  -- F-004 corruption arriving by a route with no concurrency in it at all.
  --
  -- DELIBERATELY AFTER the FOR UPDATE above. That lock is what stops a pair
  -- being inserted for either account between this check and the delete: an
  -- INSERT's foreign key check takes FOR KEY SHARE on the parent row, and
  -- FOR UPDATE conflicts with it. Checked before the lock, this would be a
  -- read that anything could invalidate.
  --
  -- The collision case REFUSES rather than dropping the loser's pair. Dropping
  -- it is what the participant path does one block below, and it is wrong here:
  -- tournament_matches references pairs with NO ACTION too, so a pair already
  -- in a draw cannot be deleted at all, and one that can be is still a real
  -- entry somebody made. Which entry survives is an operator's decision, not a
  -- merge's. The refusal names the event so it is actionable.
  SELECT t.name || ' / ' || ev.event_type
    INTO v_conflict
    FROM tournament_pairs pr
    JOIN tournament_events ev ON ev.id = pr.event_id
    JOIN tournaments t        ON t.id  = ev.tournament_id
   WHERE (pr.player1_id = p_remove OR pr.player2_id = p_remove)
     AND (
       -- the survivor is the other half of this very team
       pr.player1_id = p_keep OR pr.player2_id = p_keep
       -- or the survivor is in the same event some other way
       OR EXISTS (SELECT 1 FROM tournament_pairs k
                   WHERE k.event_id = pr.event_id
                     AND k.id <> pr.id
                     AND (k.player1_id = p_keep OR k.player2_id = p_keep))
       OR EXISTS (SELECT 1 FROM tournament_participants k
                   WHERE k.event_id = pr.event_id AND k.player_id = p_keep)
     )
   LIMIT 1;

  IF v_conflict IS NOT NULL THEN
    RAISE EXCEPTION 'Both accounts are entered in the same event (%). Remove one of the two entries from that event first, then merge.', v_conflict
      USING ERRCODE = 'check_violation';
  END IF;

  -- ---- The same-event refusal the participant path never had -- 00216 ----
  --
  -- The block above refuses a same-event collision only when the removed
  -- account is in a PAIR. Two PARTICIPANT rows in one event were never
  -- refused: the remap further down sends the discarded row's draw matches
  -- onto the survivor's row in that same event, and the discarded row is then
  -- CASCADE-deleted with the player.
  --
  -- That remap is right when only ONE of the two rows ever played -- a person
  -- who registered twice, played under one entry, and whose duplicate is the
  -- thing to discard. It is wrong when BOTH rows played: the draw seeded them
  -- as two distinct entrants, and collapsing their match references onto one
  -- participant row puts that row in two bracket slots at once.
  --
  -- Nothing downstream notices. The self-play sweep below catches only the
  -- narrower case where the two actually met each other; two entrants on
  -- opposite halves of a draw never produce a match with both sides equal.
  -- Finalisation then reads the collapsed row as occupying whichever slot it
  -- looks at last, so a single positionMap entry is overwritten and the wrong
  -- placing is published -- with the field lock held, so no later recompute
  -- can repair it.
  --
  -- Which of the two entrants keeps the draw is an operator's decision, the
  -- same reasoning the pair block gives. So this refuses and names the event.
  --
  -- Scoped deliberately to rows that PLAYED. A duplicate registration in a
  -- draft or ungenerated event has no match references, keeps dropping exactly
  -- as it does today, and stays a one-click merge -- which is the common
  -- duplicate-account case and must not become a two-step chore.
  SELECT t.name || ' / ' || ev.event_type
    INTO v_conflict
    FROM tournament_participants x
    JOIN tournament_participants k
      ON k.event_id = x.event_id AND k.player_id = p_keep
    JOIN tournament_events ev ON ev.id = x.event_id
    JOIN tournaments t        ON t.id  = ev.tournament_id
   WHERE x.player_id = p_remove
     AND EXISTS (SELECT 1 FROM tournament_matches tm
                  WHERE tm.participant_a_id      = x.id
                     OR tm.participant_b_id      = x.id
                     OR tm.winner_participant_id = x.id
                     OR tm.loser_participant_id  = x.id)
     AND EXISTS (SELECT 1 FROM tournament_matches tm
                  WHERE tm.participant_a_id      = k.id
                     OR tm.participant_b_id      = k.id
                     OR tm.winner_participant_id = k.id
                     OR tm.loser_participant_id  = k.id)
   LIMIT 1;

  IF v_conflict IS NOT NULL THEN
    RAISE EXCEPTION 'Both accounts played in the same event (%). Merging them would put one entrant in two places in that draw. Withdraw and remove one of the two entries first, then merge.', v_conflict
      USING ERRCODE = 'check_violation';
  END IF;

  UPDATE tournament_pairs SET player1_id = p_keep WHERE player1_id = p_remove;
  GET DIAGNOSTICS v_n = ROW_COUNT;
  IF v_n > 0 THEN v_kept := v_kept || jsonb_build_object('tournament_pairs_player1', v_n); END IF;

  UPDATE tournament_pairs SET player2_id = p_keep WHERE player2_id = p_remove;
  GET DIAGNOSTICS v_n = ROW_COUNT;
  IF v_n > 0 THEN v_kept := v_kept || jsonb_build_object('tournament_pairs_player2', v_n); END IF;
...
-- Verification.
--
-- Three behavioural probes, each building its own fixtures in a sub-block and
-- rolling them back by raising. A post-condition the code under test cannot
-- violate proves nothing, so the negative probes are the load-bearing ones:
-- an unconditional refusal would pass probe 1 and fail probes 2 and 3.
-- ---------------------------------------------------------------------------
DO $verify$
DECLARE
  v_a        UUID;
  ...
  v_kept_only BOOLEAN := FALSE;
  v_owner    UUID;
BEGIN
  -- 1. Both entrants played in the same event: refuse, readably.
  BEGIN
    INSERT INTO players (email, first_name) VALUES ('_mp216a@invalid.test','P216A') RETURNING id INTO v_a;
    INSERT INTO players (email, first_name) VALUES ('_mp216b@invalid.test','P216B') RETURNING id INTO v_b;
    INSERT INTO tournaments (name, start_date) VALUES ('_mp216 probe 1', CURRENT_DATE) RETURNING id INTO v_t;
    INSERT INTO tournament_events (tournament_id, event_type, format)
      VALUES (v_t, 'mens_singles', 'single_elimination') RETURNING id INTO v_e;
    INSERT INTO tournament_participants (event_id, player_id) VALUES (v_e, v_a) RETURNING id INTO v_pa;
    INSERT INTO tournament_participants (event_id, player_id) VALUES (v_e, v_b) RETURNING id INTO v_pb;
    -- Two separate first-round matches: they never met, so the self-play sweep
    -- would not have caught this.
    INSERT INTO tournament_matches (event_id, round_number, bracket_position, participant_a_id, winner_participant_id)
      VALUES (v_e, 1, 1, v_pa, v_pa);
    INSERT INTO tournament_matches (event_id, round_number, bracket_position, participant_a_id, winner_participant_id)
      VALUES (v_e, 1, 2, v_pb, v_pb);
    BEGIN
      PERFORM merge_players(v_b, v_a, v_b);
    EXCEPTION
      WHEN check_violation THEN v_refused := TRUE;
      WHEN foreign_key_violation THEN
        RAISE EXCEPTION '00216: the collapse died on a foreign key instead of refusing readably';
    END;
    RAISE EXCEPTION 'rollback probe 1';
  EXCEPTION WHEN raise_exception THEN
    IF SQLERRM <> 'rollback probe 1' THEN RAISE; END IF;
  END;

  -- 2. Only the discarded entry played: the merge still goes through and the
  --    matches land on the survivor. This is 00207's remap and it must survive.
  BEGIN
    INSERT INTO players (email, first_name) VALUES ('_mp216c@invalid.test','P216C') RETURNING id INTO v_a;
    INSERT INTO players (email, first_name) VALUES ('_mp216d@invalid.test','P216D') RETURNING id INTO v_b;
    INSERT INTO tournaments (name, start_date) VALUES ('_mp216 probe 2', CURRENT_DATE) RETURNING id INTO v_t;
    INSERT INTO tournament_events (tournament_id, event_type, format)
      VALUES (v_t, 'mens_singles', 'single_elimination') RETURNING id INTO v_e;
    INSERT INTO tournament_participants (event_id, player_id) VALUES (v_e, v_a) RETURNING id INTO v_pa;
    INSERT INTO tournament_participants (event_id, player_id) VALUES (v_e, v_b) RETURNING id INTO v_pb;
    ...
    PERFORM merge_players(v_b, v_a, v_b);
    v_merged := TRUE;
    SELECT participant_a_id INTO v_owner FROM tournament_matches
     WHERE event_id = v_e AND bracket_position = 1;
    IF v_owner IS DISTINCT FROM v_pb THEN
      RAISE EXCEPTION '00216: the one-sided remap no longer lands on the survivor (participant_a_id is %)', COALESCE(v_owner::text,'NULL');
    END IF;
    RAISE EXCEPTION 'rollback probe 2';
  EXCEPTION WHEN raise_exception THEN
    IF SQLERRM <> 'rollback probe 2' THEN RAISE; END IF;
  END;

  -- 3. A duplicate registration in an event with no draw yet still merges in
  --    one click. This is the common duplicate-account case; refusing here
  --    would be a regression in the ordinary path, not a fix.
  BEGIN
    INSERT INTO players (email, first_name) VALUES ('_mp216e@invalid.test','P216E') RETURNING id INTO v_a;
    INSERT INTO players (email, first_name) VALUES ('_mp216f@invalid.test','P216F') RETURNING id INTO v_b;
    INSERT INTO tournaments (name, start_date) VALUES ('_mp216 probe 3', CURRENT_DATE) RETURNING id INTO v_t;
    INSERT INTO tournament_events (tournament_id, event_type, format)
      VALUES (v_t, 'mens_singles', 'single_elimination') RETURNING id INTO v_e;
    INSERT INTO tournament_participants (event_id, player_id) VALUES (v_e, v_a);
    INSERT INTO tournament_participants (event_id, player_id) VALUES (v_e, v_b);
    PERFORM merge_players(v_b, v_a, v_b);
    v_draft := TRUE;
  ...
  -- 4. Only the SURVIVOR played. The discarded row holds no match references,
  --    so nothing is remapped and nothing collapses. A guard keyed on the
  --    survivor alone would refuse here; this is the probe that catches it.
  BEGIN
    INSERT INTO players (email, first_name) VALUES ('_mp216g@invalid.test','P216G') RETURNING id INTO v_a;
    INSERT INTO players (email, first_name) VALUES ('_mp216h@invalid.test','P216H') RETURNING id INTO v_b;
    INSERT INTO tournaments (name, start_date) VALUES ('_mp216 probe 4', CURRENT_DATE) RETURNING id INTO v_t;
    INSERT INTO tournament_events (tournament_id, event_type, format)
      VALUES (v_t, 'mens_singles', 'single_elimination') RETURNING id INTO v_e;
    INSERT INTO tournament_participants (event_id, player_id) VALUES (v_e, v_a);
    INSERT INTO tournament_participants (event_id, player_id) VALUES (v_e, v_b) RETURNING id INTO v_pb;
    INSERT INTO tournament_matches (event_id, round_number, bracket_position, participant_a_id, winner_participant_id)
      VALUES (v_e, 1, 1, v_pb, v_pb);
    PERFORM merge_players(v_b, v_a, v_b);
    v_kept_only := TRUE;
  ...
  IF NOT v_refused THEN
    RAISE EXCEPTION '00216: a merge that collapses two played entries into one bracket slot was allowed through';
  END IF;
  IF NOT v_merged THEN
    RAISE EXCEPTION '00216: the guard over-refuses -- a duplicate entry whose matches all belong to the discarded row no longer merges';
  END IF;
  IF NOT v_draft THEN
    RAISE EXCEPTION '00216: the guard over-refuses -- a duplicate registration in an event with no draw no longer merges';
  END IF;
  IF NOT v_kept_only THEN
    RAISE EXCEPTION '00216: the guard over-refuses -- a duplicate entry whose matches all belong to the SURVIVING row no longer merges';
  END IF;

  RAISE NOTICE '00216 verified: two played entries in one event refuse by name, a one-sided remap still lands on the survivor, a duplicate registration with no draw still merges, and a duplicate whose matches all belong to the survivor still merges';
END
$verify$;
