...
CREATE TABLE ratings (
  ...
  singles_elo INTEGER NOT NULL DEFAULT 400,
  doubles_elo INTEGER NOT NULL DEFAULT 400,
  ...
  singles_k_factor INTEGER NOT NULL DEFAULT 80,
  doubles_k_factor INTEGER NOT NULL DEFAULT 64,
...
-- has since been deleted anyway. Grep for the table name before assuming any
-- application code still touches it; anything this file asserts about app
-- structure is out of date the moment somebody moves a file.
--
-- WHY THE TABLE STAYS EVEN WITH NO WRITER LEFT: it holds real historical
-- placements — production may still have pre-event-system rows — and
-- merge_players() rewrites its player_id and partner_id when two members are
-- merged (00026 first, then 00079 and 00095, each of which lists this table by
-- name). Dropping it would silently break that path and lose the history. Do
-- not add new writers; the event-based tables are where new work goes.
...
CREATE TABLE match_participants (
  ...
  player_id UUID NOT NULL REFERENCES players(id) ON DELETE CASCADE,
  team_side team_side NOT NULL,
  pre_rating INTEGER NOT NULL DEFAULT 400,
  post_rating INTEGER,
