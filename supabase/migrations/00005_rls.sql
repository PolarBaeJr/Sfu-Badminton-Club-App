...
-- Creator can update own challenges (cancel)
...
CREATE POLICY matches_insert ON matches FOR INSERT TO authenticated
  WITH CHECK (TRUE); -- Validated via server actions
...
CREATE POLICY mp_insert ON match_participants FOR INSERT TO authenticated WITH CHECK (TRUE);
...
CREATE POLICY mg_insert ON match_games FOR INSERT TO authenticated WITH CHECK (TRUE);
...
CREATE POLICY tp_admin ON tournament_participants FOR ALL TO authenticated USING (is_admin(auth.uid()));
