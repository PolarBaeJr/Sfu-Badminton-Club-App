...
CREATE POLICY tf_admin ON tournament_fees FOR ALL TO authenticated USING (is_admin(auth.uid()));
CREATE POLICY rf_select_own ON reinstatement_fees FOR SELECT TO authenticated USING (player_id = get_player_id(auth.uid()));
CREATE POLICY rf_admin ON reinstatement_fees FOR ALL TO authenticated USING (is_admin(auth.uid()));
