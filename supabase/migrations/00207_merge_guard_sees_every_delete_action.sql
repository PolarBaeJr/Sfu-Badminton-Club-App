-- 00207 — the merge guard only ever looked at one third of the problem
--
-- merge_players_unhandled() exists so that a foreign key to players that nobody
-- has classified stops a merge instead of being resolved silently. It has been
-- filtering on `c.confdeltype = 'c'` since 00079, which means it has only ever
-- reported CASCADE columns. The other two delete actions were invisible to it,
-- and both of them have a failure mode:
--
--   ON DELETE NO ACTION — the final DELETE throws. Reproduced on staging: put
--   the removed account in a doubles pair and merge_players dies with
--     23503 ... violates foreign key constraint "tournament_pairs_player1_id_fkey"
--   after having already committed a dozen repoints in the same transaction.
--   They roll back, so nothing is corrupted; but the operator gets a bare
--   Postgres error with no statement of what to do, and merge_players_unhandled()
--   -- the function whose whole job is to say "this is not classified" --
--   returned zero rows immediately beforehand.
--
--   ON DELETE SET NULL — nothing throws at all. The merge succeeds and the
--   column is blanked. This is the half that worries me more, because there is
--   no error to notice: an admin note loses its author, a dispute loses who
--   claimed it, a permission baseline loses who created it, and once the removed
--   account's id is gone the attribution cannot be reconstructed.
--
-- Census of every FK to players, taken on staging and on prod (identical apart
-- from two columns that pending migrations add), then differenced against what
-- merge_players actually touches:
--
--   NO ACTION, unhandled (5):  club_ledger.marked_by, club_ledger.paid_by,
--                              club_ledger.reimbursed_by,
--                              tournament_pairs.player1_id, .player2_id
--   SET NULL,  unhandled (11): discord_link_tokens.consumed_by,
--                              disputes.claimed_by,
--                              event_waiver_templates.updated_by,
--                              match_admin_notes.author_id,
--                              permission_baselines.created_by, .updated_by,
--                              tournament_match_notes.author_id,
--                              tournament_pair_notes.author_id,
--                              tournament_participant_notes.author_id,
--                              walkover_admin_notes.author_id
--   CASCADE,   unhandled (0):  the original filter's own class was clean.
--
-- That last line is why this was never caught. The guard was right about
-- everything it looked at.
--
-- This migration widens the guard to every delete action, classifies all
-- sixteen, and adds the repoints that make the classification true. Fifteen are
-- attribution and simply move to the survivor. The sixteenth case --
-- tournament_pairs' two identity columns -- is not attribution and is handled
-- separately in the function body, with the reasoning next to the code.
--
-- WHY THIS IS RISKY AND WHAT GUARDS IT: widening the guard means that any FK
-- this migration fails to classify will refuse EVERY merge on production, which
-- is exactly the 00165 bug that 00204 had to fix. The verification block at the
-- bottom therefore asserts the guard is empty, and then proves behaviourally
-- that it is empty for the right reason rather than because it was gutted.
...
CREATE OR REPLACE FUNCTION public.merge_players_unhandled()
 ...
 STABLE
AS $function$
  -- NO confdeltype FILTER. That filter was the bug: it hid NO ACTION columns,
  -- which make the merge throw, and SET NULL columns, which make it lose data
  -- without throwing. Anything referencing players is in scope now, and every
  -- column below has been classified by hand.
  ...
  WHERE c.contype = 'f'
    ...
    AND (c.conrelid::regclass::text, a.attname::text) NOT IN (
      ...
      UNION ALL
      -- CASCADE columns the merge either moves or deliberately lets go, with
      -- the count reported in the return value. Unchanged from 00204 except
      -- that the stale "the loser provably has none of these" comment is gone:
      -- merge_players repoints most of these and reports the remainder as
      -- discarded, which is not the same thing as the loser not having any.
      SELECT * FROM (VALUES
      ...
      UNION ALL
      -- CASCADE columns repointed so the rows survive the merge.
      SELECT * FROM (VALUES
      ...
      UNION ALL
      -- NO ACTION. Every one of these MUST be repointed or the final DELETE
      -- throws. If you add a NO ACTION reference to players and do not repoint
      -- it, this guard is what tells you -- before the merge, by name, instead
      -- of a 23503 halfway through.
      SELECT * FROM (VALUES
        ('players','banned_by'), ('announcements','author_id'),
        ('club_fees','marked_by'),
        ('club_ledger','marked_by'), ('club_ledger','paid_by'),
        ('club_ledger','reimbursed_by'),
        ('tournament_audit_log','performed_by'),
        ('tournament_matches','result_entered_by'),
        ('tournament_pairs','added_by'), ('tournament_pairs','checked_in_by'),
        ('tournament_participants','added_by'),
        ('tournament_participants','checked_in_by'),
        -- identity, not attribution: repointed only when the survivor is not
        -- already in the event, and the merge refuses when they are.
        ('tournament_pairs','player1_id'), ('tournament_pairs','player2_id')
      ) AS no_action(tbl, col)
      UNION ALL
      -- SET NULL. These never threw, which is why they went unnoticed for so
      -- long. All attribution; all repointed to the survivor, who is the same
      -- human being.
      SELECT * FROM (VALUES
        ('audit_logs','actor_id'), ('disputes','resolved_by'),
        ('disputes','claimed_by'),
        ('discord_link_tokens','consumed_by'),
        ('event_waiver_templates','updated_by'),
        ('feedback_reports','player_id'),
        ('legacy_tournament_participants','partner_id'),
        ('legal_documents','updated_by'),
        ('match_admin_notes','author_id'),
        ('matches','confirmed_by'), ('matches','forfeit_player_id'),
        ('matches','submitted_by'),
        ('permission_baselines','created_by'), ('permission_baselines','updated_by'),
        ('platform_settings','updated_by'),
        ('session_attendance','marked_by'), ('sessions','host_player_id'),
        ('tournament_match_notes','author_id'),
        ('tournament_pair_notes','author_id'),
        ('tournament_participant_notes','author_id'),
        ('tournaments','created_by'),
        ('walkover_admin_notes','author_id'),
        ('walkovers','admin_confirmed_by')
      ) AS set_null(tbl, col)
    );
$function$;
...
-- ---------------------------------------------------------------------------
-- merge_players, restated from 00205 with the sixteen columns handled.
-- ---------------------------------------------------------------------------
...
-- Verification.
--
-- Four checks. The first is a state assertion and the other three are
-- behavioural, because 00190 and the first draft of 00206 both taught the same
-- lesson: a post-condition that the code under test cannot violate proves
-- nothing. Each probe below builds its own fixtures in a sub-block and rolls
-- them back by raising, so the migration leaves no rows behind.
-- ---------------------------------------------------------------------------
DO $verify$
DECLARE
  v_gap        TEXT;
  ...
  v_pair       UUID;
  v_p1         UUID;
  v_saw_guard  BOOLEAN := FALSE;
  v_refused    BOOLEAN := FALSE;
  v_consumed   UUID;
BEGIN
  -- 1. The guard is empty. If it is not, this migration has misclassified
  --    something and EVERY merge on this database is about to start refusing,
  --    so name the columns rather than just failing.
  SELECT string_agg(tbl || '.' || col, ', ' ORDER BY tbl, col)
    INTO v_gap FROM merge_players_unhandled();
  IF v_gap IS NOT NULL THEN
    RAISE EXCEPTION '00207: unclassified foreign keys to players remain: %', v_gap;
  END IF;

  -- 2. The guard is empty for the right reason. A guard gutted to return
  --    nothing would also pass check 1, so give it a genuinely unclassified
  --    reference and require it to report it. The table is created and dropped
  --    inside a sub-block; nothing survives.
  BEGIN
    CREATE TABLE _mp_guard_probe (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
                                  player_id uuid REFERENCES players(id) ON DELETE SET NULL);
    SELECT EXISTS (SELECT 1 FROM merge_players_unhandled()
                    WHERE tbl = '_mp_guard_probe' AND col = 'player_id')
      INTO v_saw_guard;
    RAISE EXCEPTION 'rollback probe 2';
  EXCEPTION WHEN raise_exception THEN
    IF SQLERRM <> 'rollback probe 2' THEN RAISE; END IF;
  END;
  IF NOT v_saw_guard THEN
    RAISE EXCEPTION '00207: merge_players_unhandled() did not report an unclassified SET NULL reference. The guard is not looking at what it claims to look at.';
  END IF;

  -- 3. A removed account that is half of a doubles team merges, and the team
  --    now names the survivor. Before 00207 this raised 23503.
  BEGIN
    INSERT INTO players (email, first_name) VALUES ('_mp207a@invalid.test','ProbeA') RETURNING id INTO v_a;
    INSERT INTO players (email, first_name) VALUES ('_mp207b@invalid.test','ProbeB') RETURNING id INTO v_b;
    INSERT INTO players (email, first_name) VALUES ('_mp207c@invalid.test','ProbeC') RETURNING id INTO v_c;
    INSERT INTO tournaments (name, start_date) VALUES ('_mp207 probe', CURRENT_DATE) RETURNING id INTO v_t;
    INSERT INTO tournament_events (tournament_id, event_type, format)
      VALUES (v_t, 'mens_doubles', 'single_elimination') RETURNING id INTO v_e;
    -- loser A partners C; survivor B is not in this event at all.
    INSERT INTO tournament_pairs (event_id, player1_id, player2_id)
      VALUES (v_e, v_a, v_c) RETURNING id INTO v_pair;

    -- a SET NULL column owned by the loser, to prove attribution moves rather
    -- than being blanked.
    INSERT INTO discord_link_tokens (token_hash, discord_user_id, expires_at, consumed_by)
      VALUES ('_mp207probe', '_mp207probe', now() + interval '1 hour', v_a);

    PERFORM merge_players(v_b, v_a, v_b);

    SELECT player1_id INTO v_p1 FROM tournament_pairs WHERE id = v_pair;
    IF v_p1 <> v_b THEN
      RAISE EXCEPTION '00207: pair still names % after the merge, expected the survivor %', v_p1, v_b;
    END IF;

    SELECT consumed_by INTO v_consumed FROM discord_link_tokens WHERE token_hash = '_mp207probe';
    IF v_consumed IS DISTINCT FROM v_b THEN
      RAISE EXCEPTION '00207: SET NULL attribution was not moved to the survivor -- consumed_by is %', COALESCE(v_consumed::text, 'NULL');
    END IF;
  ...
  -- 4. Both accounts in the same event refuses, and refuses with the readable
  --    message rather than a foreign key violation.
  BEGIN
    INSERT INTO players (email, first_name) VALUES ('_mp207d@invalid.test','ProbeD') RETURNING id INTO v_a;
    INSERT INTO players (email, first_name) VALUES ('_mp207e@invalid.test','ProbeE') RETURNING id INTO v_b;
    INSERT INTO tournaments (name, start_date) VALUES ('_mp207 probe 2', CURRENT_DATE) RETURNING id INTO v_t;
    INSERT INTO tournament_events (tournament_id, event_type, format)
      VALUES (v_t, 'mens_doubles', 'single_elimination') RETURNING id INTO v_e;
    INSERT INTO tournament_pairs (event_id, player1_id, player2_id) VALUES (v_e, v_a, v_b);
    BEGIN
      PERFORM merge_players(v_b, v_a, v_b);
    EXCEPTION
      WHEN check_violation THEN v_refused := TRUE;
      WHEN foreign_key_violation THEN
        RAISE EXCEPTION '00207: same-event merge still dies on a foreign key instead of refusing readably';
    END;
  ...
  IF NOT v_refused THEN
    RAISE EXCEPTION '00207: a merge of two accounts entered in the same event was allowed through';
  END IF;

  RAISE NOTICE '00207 verified: guard covers all three delete actions and reports an unclassified reference; a paired account merges and keeps its attribution; a same-event merge refuses readably';
END
$verify$;
