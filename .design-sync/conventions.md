...
**Setup.** No provider or wrapper is required. Import components from `window.BadmintonUI` (e.g. `const { Button, Card, PageHeader } = window.BadmintonUI`). The stylesheet ships dark-theme tokens on `:root`; design on the dark slate ground it provides (`--bg` #0B0F1A, cards `--surface` #151C2C), with the club red `--red` #E94560 as the single accent.
