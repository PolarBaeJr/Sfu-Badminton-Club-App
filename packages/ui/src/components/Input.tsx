    ...
    return (
      <div className="space-y-1">
        ...
        <input
          ...
          className={cn(
            // rounded-[var(--r-control,8px)], not rounded-[8px]: an arbitrary
            // value is the one radius that SURVIVES both apps' zeroed Tailwind
            // scale, so a literal 8px here is 8px in the admin console, whose
            // guideline is radius 0 outside dialogs. The console sets
            // --r-control: 0 in its globals.css; the player app sets nothing
            // and takes the 8px fallback, so it is unchanged by construction.
            'w-full px-3 min-h-[48px] bg-[var(--bg-surface)] border border-[var(--border)] rounded-[var(--r-control,8px)] text-[var(--text-primary)] placeholder-[var(--text-muted)] focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)] focus:border-transparent transition-colors disabled:opacity-50 disabled:cursor-not-allowed',
